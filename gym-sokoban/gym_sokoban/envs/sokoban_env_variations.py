...
from .sokoban_env_stochastic import StochasticSokobanEnv


class SokobanEnv1ray(SokobanEnv):
    metadata = {
        'render.modes': ['human', 'rgb_array', 'tiny_human', 'tiny_rgb_array'],
    }

    def __init__(self, env_config):
        super(SokobanEnv1ray, self).__init__(
            num_boxes=env_config["num_boxes"], max_steps=env_config["max_steps"], seed=env_config["seed"], fixed_env=env_config["fixed_env"], randomized_init_position=env_config["randomized_init_position"]
        )
...
class SokobanEnv1(SokobanEnv):
    ...
    def __init__(self, num_boxes, max_steps, index, seed, fixed_env,randomized_init_position):
        super(SokobanEnv1, self).__init__(
            num_boxes=num_boxes, max_steps=max_steps, index=index, seed=seed, fixed_env=fixed_env, randomized_init_position=randomized_init_position
        )

class SokobanEnv_Stochastic(StochasticSokobanEnv):
    metadata = {
        'render.modes': ['human', 'rgb_array', 'tiny_human', 'tiny_rgb_array'],
    }

    def __init__(self, num_boxes, max_steps, seed, fixed_env,randomized_init_position):
        super(SokobanEnv_Stochastic, self).__init__(
            num_boxes=num_boxes, max_steps=max_steps, seed=seed, fixed_env=fixed_env, randomized_init_position=randomized_init_position
        )

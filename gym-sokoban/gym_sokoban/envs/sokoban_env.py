...
class SokobanEnv(gym.Env):
    ...
    def __init__(self,
        ...
        # if fixed_env:
        #     try:
        #         s = load_obj(f'state_sokoban{index}')
        #         self.init_state(s)
        #     except:
        #         import pdb; pdb.set_trace()
        #         print("no saved initial state, creating one...")
        #         s = self.get_state()
        #         save_obj(s, f'state_sokoban{index}')
    ...
    def get_state(self):
        return {"num_env_steps": self.num_env_steps, "room_fixed":np.copy(self.room_fixed), "room_state":np.copy(self.room_state), "box_mapping":copy.copy(self.box_mapping), "player_position":np.copy(self.player_position), "reward_last":self.reward_last, "boxes_on_target":self.boxes_on_target}

    def init_state(self, state):
        self.num_env_steps, self.room_fixed, self.room_state, self.box_mapping, self.player_position, self.reward_last, self.boxes_on_target = state["num_env_steps"], np.copy(state["room_fixed"]), np.copy(state["room_state"]), copy.copy(state["box_mapping"]), np.copy(state["player_position"]), state["reward_last"], state["boxes_on_target"]
    ...
    def reset(self, second_player=False, render_mode='rgb_array'):
        if self.fixed_env:
            try:
                s = load_obj(f'state_sokoban{self.index}')
                self.init_state(s)
            except:
                try:
                ...
                s = self.get_state()
                # import pdb; pdb.set_trace()
                save_obj(s, f'state_sokoban{self.index}')
            if self.randomized_init_position:
                # Get all available positions
                self.room_state[self.room_state == 5] = 1
                possible_positions = np.where(self.room_state == 1)
                ...
                self.room_state[player_position] = 5
                self.player_position = np.argwhere(self.room_state == 5)[0]

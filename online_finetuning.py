...
parser.add_argument('--steps_finetune', type=int, default=64)
parser.add_argument('--nb_gradient_finetune', type=int, default=16)
parser.add_argument('--min_horizon_online_search', type=int, default=5)
...
env_id = args.env
env = gym.make(env_id, num_boxes= args.num_boxes, index = args.index_level, max_steps=args.test_ep_length, seed=args.seed, fixed_env=fixed_env, randomized_init_position=randomized_init_position)
...
def vanilla(params_finetune_trainer, env_id, params_env,nb_episode, max_ep_len, path_ac, path_piot, path_vopt):
    ...
    for episode in range(nb_episode):
        ...
        while True:
            ...
            if done or ep_len>max_ep_len:
                ...
                break
    wandb.log({'Vanilla Test Cumulative Reward': np.mean(cumulative_rewards), 'Finetune Test Cumulative Reward': np.mean(cumulative_rewards), "Online Samples": 0, "Time": 0, "Online planning minimum horizon":0, "Number of online gradient descent per step":0, "Online batch size":0})

def finetune(params_finetune_trainer, min_horizon, online_samples, env_id, params_env, nb_episode, max_ep_len, path_ac, path_piot, path_vopt, cupt=5):
    test_env = gym.make(env_id, **params_env)
        ...
        while sample<online_samples:
            while simulation_trainer.buf.ptr < simulation_trainer.buf.max_size:
                for first_action in range(planning_env.action_space.n):
                    ...
                    length_seq+=1
                    while length_seq<min_horizon or tuple(action_seq) in action_seq_seen_so_far:
    ...
    for episode in range(nb_episode):
        ...
        while True:
            ...
            if done or ep_len>max_ep_len:
                ...
                break
            state = test_env.get_state()
    wandb.log({'Finetune Test Cumulative Reward': np.mean(cumulative_rewards), "Online Samples": np.mean(samples), "Time": np.mean(times), "Online planning minimum horizon":min_horizon, "Number of online gradient descent per step":simulation_trainer.train_pi_iters, "Online batch size":simulation_trainer.local_steps_per_epoch})

nb_online_samples = [128,512,1024]
